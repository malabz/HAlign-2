...
public class ExtremeMSA {
	...
	public boolean start(String inputFile, String outputFile, String outputDFS)
		...
		long timeDP = System.currentTimeMillis();
		output(combine(), outputFile);
		long timeOutput = System.currentTimeMillis();
		System.out.printf("input time: %s\n", timeInput - start);
		System.out.printf("align time: %s\n", timeAlign - timeInput);
		System.out.printf("dp time: %s\n", timeDP - timeAlign);
		System.out.printf("output time: %s\n", timeOutput - timeDP);
		return true;
	}
	...
	private int count() {
		...
		if (n == 0) {
			...
			System.out.println("only 1 sequence found");
